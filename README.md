...
- [x] **SERVIDOR NA VPS (2026-08-01):** VPS `187.77.36.21` (mesma do site de inglês, hostname srv1359505), Docker Compose em `/opt/aldruna/` — MariaDB + nosso login server (porta **8081**, porque a 8080 é do nginx do site) + servidor em imagem própria (`/opt/aldruna/server/`). **CONFIRMADO PELO JULIO 2026-08-01: jogando online no mundo pela conta @god.** Container names ainda `aldruna-*`.
  - **ARMADILHA (2026-08-01): a imagem `ghcr.io/opentibiabr/canary:3.6.1` MENTE** — foi reconstruída com código mais novo e fala protocolo **15.25**, enquanto a v3.6.1 real (PC + cliente) fala **15.11**. Sintoma no cliente: janela "Editar Texto" com bytes embaralhados após escolher personagem, depois "conectando para sempre". Solução: imagem própria com o **binário oficial** `canary-linux-release.zip` da release v3.6.1 do GitHub + datapack `data-canary` do fonte v3.6.1 + cópia do config.lua do PC (mapa `canary.otbm`, o mesmo do local). Conferir SEMPRE a linha "Server protocol: 15.11" no log ao mexer no servidor da VPS.
  ...
  - **Conta de jogador do Julio (2026-08-01):** email `julioamancio2014@gmail.com` (senha combinada no chat), com Julio Warrior / Ranger / Arcanist / Druid / Monk, todos level 100, skills/magic prontos para teste, magias needLearn liberadas via `player_spells`. Existe igual nos DOIS bancos (PC e VPS). Receita de replicação: copiar linha do `<Voc> Sample` via TEMPORARY TABLE + ajustar ganhos/nível.
- [x] **Vocações Destruitor (2026-08-01):** Knight→Warrior, Paladin→Ranger, Sorcerer→Arcanist, Druid fica (promoções Elite Warrior / Royal Ranger / Master Arcanist / Elder Druid). Alterado em `data/XML/vocations.xml` + 137 arquivos Lua (linhas `spell:vocation(...)`/`rune:vocation(...)`) + descrições de poções + emojiMap de player.lua + dict VOCATIONS do `ot-tools/login_server.py` + rename dos personagens Sample nos DOIS bancos. Aplicado no PC e na VPS (datapack vai por tar/scp para `/opt/aldruna/server/` + `docker compose build`). Boot limpo nos dois (sem "Wrong vocation"). Monk/Exalted Monk mantidos por enquanto (decidir depois se ficam). Constantes internas (`VOCATION.BASE_ID.KNIGHT`, `Player.isKnight()`) NÃO foram renomeadas de propósito — são identificadores por id, invisíveis ao jogador.
- [x] **Runespeech (2026-08-01, v2 intuitiva):** TODAS as palavras mágicas da CipSoft substituídas — a pedido do Julio as raízes são INTUITIVAS em inglês (exura→`mend`, exori→`strike`, exevo flam→`wave fire`, utevo lux→`conjure light`). Dicionário completo em `docs/runespeech.md`; guia de magias por vocação (gerado dos scripts) em `docs/magias-por-vocacao.md`. Aplicado no servidor (197 arquivos, `:words(...)`) e no cliente (`modules/gamelib/spells.lua`). Magia nova = usar só raízes do dicionário. Boot limpo no PC e na VPS.
- [x] **Nomes das magias (2026-08-01):** 95 nomes da CipSoft trocados por nomes descritivos que casam com as palavras (Buzz→Novice Energy Strike, Animate Dead Rune→Raise Dead Rune, Haste→Speed, Sorcerer familiar→Arcanist Familiar...). Guia regenerado em `docs/magias-por-vocacao.md`. Renomeados no servidor (`:name`), no cliente (chave + campo `name` de spells.lua) e no `player_spells` dos dois bancos.
  - **BLOQUEIO CONHECIDO — 53 nomes só mudam com binário próprio:** nomes como Avatar of Steel, Annihilation, Berserk, Magic Shield, Ultimate Healing, Divine Caldera, Hell's Core estão HARDCODED no C++ (`src/creatures/players/components/wheel/player_wheel.cpp` e afins — sistema Wheel of Destiny). Renomear só no Lua quebra bônus da roda silenciosamente. Lista completa: rodar o detector (grep dos nomes em `src/`). Fazer junto com a compilação do nosso binário, que é pré-requisito para abrir ao público de qualquer forma.
- **Magias precisam ser APRENDIDAS (needLearn/NPC ou `player_spells`):** personagem comum recebe "You need to learn this spell" — liberar via INSERT em `player_spells` (script pronto: receita no histórico; os 5 "Julio *" têm todas). ATENÇÃO: o servidor REGRAVA `player_spells` no logout — aplicar grants só com o personagem deslogado.
- [ ] Depois: (1) mapa próprio no Remere's, (2) troca dos assets CipSoft por arte própria ANTES de abrir ao público, (3) nomes de monstros/NPCs/itens próprios, (4) domínio www.destruitor.com.br apontando para o jogo.
